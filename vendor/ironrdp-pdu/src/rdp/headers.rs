...
const SHARE_CONTROL_HEADER_SIZE: usize = SHARE_CONTROL_HEADER_WIRE_SIZE + 4;
...
impl<'de> Decode<'de> for ShareControlHeader {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        // The wire header is six bytes; shareId belongs to the PDU body.
        // xrdp sends header-only Deactivate All PDUs without a shareId.
...
mod tests {
    ...
    #[test]
    fn decode_header_only_deactivate_all_pdu() {
        let encoded = [
            0x06, 0x00, // totalLength: six-byte Share Control wire header only
            0x16, 0x00, // Deactivate All + protocol version
            0xEE, 0x03, // pduSource
        ];

        let decoded: ShareControlHeader =
            ironrdp_core::decode(&encoded).expect("header-only Deactivate All PDU");
