...
fn build_vendored(link_mode: LinkMode) {
    ...
    // library is compiled for the `x86_64_v3` baseline (AVX2/BMI2/FMA — every
    // x86-64 CPU since ~2013/Haswell) instead of the build host's native
    // microarchitecture. A native build on a bleeding-edge CPU (e.g. Zen 5)
    // emits AVX-512/newer instructions that crash `STATUS_ILLEGAL_INSTRUCTION`
    // (0xc000001d) on older-but-still-recent x86-64 targets. Overridable via
    // `LIBGHOSTTY_VT_SYS_CPU` (any Zig CPU model: `native`, `baseline`,
    // `x86_64_v2`, `x86_64_v4`, …). Non-x86 targets keep zig's native default.
    if let Some(cpu) = zig_cpu_model(&target) {
...
/// ConMan patch: decide the Zig `-Dcpu` model for a *portable* build.
///
/// Returns `None` (zig's native default) unless the target is x86-64, in which
/// case it defaults to `x86_64_v3` so a binary built on a very new CPU still
/// runs on any Haswell-or-newer x86-64 machine. `LIBGHOSTTY_VT_SYS_CPU`
/// overrides the model for any target (empty value = fall back to native).
/// x86-only `x86_64_v*` model names must not leak to aarch64/other arches,
/// hence the arch gate.
fn zig_cpu_model(target: &str) -> Option<String> {
    if let Ok(cpu) = env::var("LIBGHOSTTY_VT_SYS_CPU") {
        return (!cpu.is_empty()).then_some(cpu);
    }
    if target.starts_with("x86_64") {
        Some("x86_64_v3".to_string())
    } else {
        None
    }
}
