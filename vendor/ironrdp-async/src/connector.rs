...
use ironrdp_connector::{
    ClientConnector, ClientConnectorState, ConnectionResult, ConnectorResult, ServerName,
};
#[cfg(feature = "credssp")]
use ironrdp_connector::{ConnectorError, State as _, general_err};
...
use crate::single_sequence_step;
...
/// Finalize the RDP connection (without CredSSP / NLA).
///
/// Available when the `credssp` feature is disabled. In this mode, only TLS
/// security (graphical login) is supported.
...
/// Finalize the RDP connection (with CredSSP / NLA support).
///
/// Available only when the `credssp` feature is enabled.
#[cfg(feature = "credssp")]
#[instrument(skip_all)]
