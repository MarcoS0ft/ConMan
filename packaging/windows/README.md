...
- `conman-<version>-windows-x86_64.zip` is the standalone distribution containing
  `conman.exe`, `conmanctl.exe`, and `ghostty-vt.dll`.
...
NSIS 3 and Python 3 must be available. The script also accepts explicit
`-MakeNsis` and `-Python` paths for provisioned runners. Official CI should run
